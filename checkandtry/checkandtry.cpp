...
public:
    ...
    void orderFood(Admin& admin) {
        ...
        for (int i = 0; i < admin.foodCount; i++) {
            ...
            for (int k = 0; k < admin.foods[i].ingredientCount; k++) {
                for (int m = 0; m < admin.ingredientCount; m++) {
                    if (admin.foods[i].ingredients[k].name == admin.ingredients[m].name) {
                        if (admin.foods[i].ingredients[k].count > admin.ingredients[m].count) {
            ...
            if (enoughIngredients) {
                cout << Foods + 1 << ". " << admin.foods[i].name << endl;
        ...
        int numItems;
        cout << "Enter total number of food to add to basket (else 0): ";
        cin >> numItems;
        cin.ignore();
        ...
        for (int i = 0; i < numItems; i++) {
            ...
            if (foodChoice >= 1 && foodChoice <= Foods) {
                int foodIndex = 0;
                int validIndex = 0;
                ...
                int foodPrice = 0;
                for (int j = 0; j < admin.foods[foodIndex].ingredientCount; j++) {
                    foodPrice += (admin.foods[foodIndex].ingredients[j].count * admin.foods[foodIndex].ingredients[j].price);
                }
                orderPrice += foodPrice;
                addfoodNames(admin.foods[foodIndex].name);
                cout << "Food added to basket" << endl;
                Sleep(2000);
                system("cls");
            }
        }
        admin.budget += (orderPrice - (10 * numItems));
        ...
        for (int i = 0; i < user.foodNamesSize; i++) {
            for (int j = 0; j < admin.foodCount; j++) {
                if (user.foodNames[i] == admin.foods[j].name) {
                    ...
                    for (int k = j; k < admin.foodCount - 1; k++) {
                        admin.foods[k] = admin.foods[k + 1];
                    }
                    admin.foodCount--;
                    break; 
                }
